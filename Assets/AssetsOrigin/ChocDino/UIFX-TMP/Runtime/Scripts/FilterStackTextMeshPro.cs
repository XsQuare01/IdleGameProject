...
namespace ChocDino.UIFX
{
	...
	[AddComponentMenu("UI/Chocolate Dinosaur UIFX/Filters/UIFX - Filter Stack (TextMeshPro)", 100)]
	public class FilterStackTextMeshPro : UIBehaviour
	{
		...
		void CalculateScreenRect()
		{
			...
			if (_filters != null)
			{
				...
				{
					if (filter && filter.isActiveAndEnabled)
					{
		...
		protected virtual void Update()
		{
			...
			if (_updateOnTransform)
			{
				...
				if (_textMeshPro.canvas && _textMeshPro.canvas.renderMode == RenderMode.WorldSpace)
				{
					...
					if (camera)
					{
						if (MathUtils.HasMatrixChanged(_previousCameraMatrix, camera.transform.localToWorldMatrix, false))
						{
