	...
	public class TrailEffectTMP : TrailEffectBase
	{
			...
			if (IsTrackingVertices())
			{
				...
				for (int j = 0; j < _layerCount; j++)
				{
					...
					if (s_isUV0Vector4)
					{
						for (int i = 0; i < vertexCount; i++)
						{
							sourceVertices.uvs0_v4[i] = targetVertices.uvs0_v4[i];
						}
					}
					else
					{
						for (int i = 0; i < vertexCount; i++)
						{
							sourceVertices.uvs0[i] = targetVertices.uvs0[i];
						}
					}
		...
		void GenerateTrailGeometry()
		{
			...
			for (int j = (_layerCount - 1); j >= 0; j--)
			{
				...
				if (IsTrackingTransform())
				{
					...
					if (_outputVerts.uvs0 != null) { System.Array.Copy(vertices.uvs0, 0, _outputVerts.uvs0, vertexOffset, layerVertexCount); }
					if (_outputVerts.uvs0_v4 != null) { System.Array.Copy(vertices.uvs0_v4, 0, _outputVerts.uvs0_v4, vertexOffset, layerVertexCount); }
				...
				else
				{
					...
					if (_outputVerts.uvs0 != null) { System.Array.Copy(layer.vertices.uvs0, 0, _outputVerts.uvs0, vertexOffset, layerVertexCount); }
					if (_outputVerts.uvs0_v4 != null) { System.Array.Copy(layer.vertices.uvs0_v4, 0, _outputVerts.uvs0_v4, vertexOffset, layerVertexCount); }
			...
			if (!_showTrailOnly)
			{
				...
				if (_originalVerts.uvs0 != null) { System.Array.Copy(_originalVerts.uvs0, 0, _outputVerts.uvs0, vertexOffset, layerVertexCount); }
				if (_originalVerts.uvs0_v4 != null) { System.Array.Copy(_originalVerts.uvs0_v4, 0, _outputVerts.uvs0_v4, vertexOffset, layerVertexCount); }
		...
		void AssignTrailGeometryToMesh(TMP_TextInfo textInfo)
		{
			...
			if (textInfo.materialCount == 1)
			{
				...
				if (_outputVerts.uvs0 != null)
				{
					meshInfo.mesh.SetUVs(0, _outputVerts.uvs0);
				}
				else if (_outputVerts.uvs0_v4 != null)
				{
			...
			else if (textInfo.materialCount > 1)
			{
				...
				for (int i = 0; i < textInfo.materialCount; i++)
				{
					...
					for (int jj = 0; jj < layerCount; jj++)
					{
						...
						if (_outputVerts.uvs0 != null) { System.Array.Copy(_outputVerts.uvs0, ll, meshInfo.uvs0, kk, oldQuadCount * 4); }
						if (_outputVerts.uvs0_v4 != null) { System.Array.Copy(_outputVerts.uvs0_v4, ll, meshInfo.uvs0, kk, oldQuadCount * 4); }
