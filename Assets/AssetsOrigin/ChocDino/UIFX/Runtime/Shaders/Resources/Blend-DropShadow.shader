Shader "Hidden/ChocDino/UIFX/Blend-DropShadow"
{
	...
	float4 fragDropShadow(v2f i) : SV_Target
	{
		...
		float4 color = lerp(0.0, source, _SourceAlpha);

		#ifdef UNITY_COLORSPACE_GAMMA
		float4 shadowColor = ToPremultiplied(_ShadowColor);
		#else
		float4 shadowColor = StraightGammaToPremultipliedLinear(_ShadowColor);
		#endif
